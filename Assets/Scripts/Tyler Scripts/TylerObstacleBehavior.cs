...
public class TylerObstacleBehavior : MonoBehaviour
{
    ...
    void Update()
    {
        if(this.tag == "BusinessPerson"&& this.transform.position.z > 0&&!moving){
            ...
            if(GameObject.Find("PlayerCharacter").transform.position.z+25 >= this.transform.position.z&&!ratMoving){
            ratMoving = true;
            StartCoroutine(ratMove());
            }
        } else if(this.tag == "Horse"&& this.transform.position.z > 0){
            if(GameObject.Find("PlayerCharacter").transform.position.z+30 >= this.transform.position.z&&!horseMoving)
            StartCoroutine(horseMove());
         } else if(this.tag == "Pigeon"&&this.transform.position.z>0){
             if(GameObject.Find("PlayerCharacter").transform.position.z+25 >= this.transform.position.z&&!pigeonMoving){
            pigeonMoving = true;
            StartCoroutine(pigeonMove());
            }
         } else if(this.tag == "Carriage"&& this.transform.position.z > 0){
            if(GameObject.Find("PlayerCharacter").transform.position.z+30 >= this.transform.position.z&&!carriageMoving)
            carriageMoving = true;
            StartCoroutine(carriageMove());
         } else if(this.tag =="Car"&&this.transform.position.z >0){
              if(GameObject.Find("PlayerCharacter").transform.position.z+30 >= this.transform.position.z&&!carMoving)
            carMoving = true;
    ...
    private IEnumerator ratMove(){
        if(this.transform.position.x == 7){
           GetComponent<Rigidbody>().velocity = new Vector3(-8*TylerMovementScript.acceleration,0,0);
    ...
    private IEnumerator catMove(){
        ...
        GetComponent<Rigidbody>().velocity = new Vector3(0,0,-8*TylerMovementScript.acceleration);
        yield return new WaitForSeconds(.5f/TylerMovementScript.acceleration);
    ...
    private IEnumerator personMove(){
        if(!moving){
        GetComponentInChildren<Animator>().speed = TylerMovementScript.acceleration;
        GetComponent<Rigidbody>().velocity = new Vector3(0,0,-9*TylerMovementScript.acceleration);
        if(GameObject.Find("PlayerCharacter").transform.position.z > 4000&&!moving){
            if(Random.Range(0,3) == 0){
                moving = true;
                yield return new WaitForSeconds(Random.Range(.5f, 1));
                if(this.transform.position.x == 7){
                    GetComponent<Rigidbody>().velocity = new Vector3(-8*TylerMovementScript.acceleration,0,-9*TylerMovementScript.acceleration);
                    yield return new WaitForSeconds(.5f/TylerMovementScript.acceleration);
                } else if(this.transform.position.x == -1){
                    GetComponent<Rigidbody>().velocity = new Vector3(8*TylerMovementScript.acceleration,0,-9*TylerMovementScript.acceleration);
                    yield return new WaitForSeconds(.5f/TylerMovementScript.acceleration);
                    ...
                    GetComponent<Rigidbody>().velocity = new Vector3(-8*TylerMovementScript.acceleration,0,-9*TylerMovementScript.acceleration);
                    yield return new WaitForSeconds(2/TylerMovementScript.acceleration);
    ...
    private IEnumerator horseMove(){
       if(!horseMoving){
        ...
        yield return new WaitForSeconds(5/TylerMovementScript.acceleration);
       Destroy(this.gameObject);
       }
    }
    ...
    private IEnumerator pigeonMove(){
        if(Random.Range(1,3)%2 ==0){
            ...
            pigeonMoving= true;
            GetComponent<Rigidbody>().velocity = new Vector3(0,5*TylerMovementScript.acceleration,0);
            this.GetComponent<Animator>().Play("bird_liftoff_001");
            yield return new WaitForSeconds(.25f/TylerMovementScript.acceleration);
